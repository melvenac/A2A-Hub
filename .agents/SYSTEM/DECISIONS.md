...
- **Consequences:** No migration, no client changes, AUTH_MODE stays warn. JSON-RPC named addressing (`metadata.to`) is an enforcement gap until identity is available there. Hub-talk rooms keep working because no peer has an `askPolicy` today.
