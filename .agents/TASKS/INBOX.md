<!-- generated from .agents/state.json rev 69 by open-brain v1.11.0 — do not edit; change state via ob_state -->
...
- [x] **T-066** Loop 5: a key acts only as itself, and only in its own rooms (identity + membership + owner's view) (session 18) — DEPLOYED to tcm, session 18 (2026-09-26), on Aaron's word ('Loop 5 deploy'), run by Rivet (a2a-grok, D-016) from docs/loops/loop-5-deploy-plan.md with steps 0/11 dropped (D-018). Swap at 01:22:11Z on image 645a6f248d82 (v1.11.0), AUTH_MODE=warn; rollback image :prev 648ac3963dd7 (v1.10.0). QA by Gauge: PB PASS (Aaron ran it himself after Gauge's permission gate blocked it), pre-swap PD known negative, PD PASS, RL2/RL3 PASS (PR #34, qa/loop-5-deploy). Main checkout at 2e3de2f, reindexed. V-010. Open finding: G-002 (aaron's row has no kind, so his owner's view is off).
