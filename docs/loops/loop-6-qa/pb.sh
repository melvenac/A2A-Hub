#!/usr/bin/env bash
# PB for the Loop 6 deploy, on the built image, before the swap. Loop 5's PB (loop-5-qa/pb.sh: PB0-PB3,
# PB2.app, PB2.no-env) plus PB1b (loop-6-qa-criteria.md D5): the image's Config.Env carries
# NODE_ENV=production. Only that one entry is printed, never the rest of Config.Env.
...
# PB1b: NODE_ENV=production in the image's Config.Env. One entry per line, END sentinel so an empty
# answer is not read as "absent"; only the NODE_ENV line is kept, and the count of other lines.
ENVL="$("${SSH[@]}" "docker image inspect --format '{{range .Config.Env}}{{println .}}{{end}}END' $ID" | tr -d '\r')"; erc=$?
NE="$(printf '%s\n' "$ENVL" | grep -E '^NODE_ENV=' || true)"; OTHER=$(printf '%s\n' "$ENVL" | grep -v -E '^NODE_ENV=|^END$' | grep -c .)
if [ $erc -ne 0 ] || [ "$(printf '%s\n' "$ENVL" | tail -1)" != END ]; then verdict PB1b no "Config.Env read failed (rc $erc); undetermined"
elif [ "$NE" = "NODE_ENV=production" ]; then verdict PB1b ok "Config.Env has NODE_ENV=production ($OTHER other entries, not printed)"
else verdict PB1b no "NODE_ENV entry is [${NE:-absent}] ($OTHER other entries, not printed)"; fi
