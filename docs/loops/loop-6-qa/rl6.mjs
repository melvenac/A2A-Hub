// RL for the Loop 6 deploy (loop-6-qa-criteria.md B6; Loop 5's RL2 in loop-5-qa/rl2.mjs): after the
// candidate's a2a-readonly and a2a-k7.mjs are installed on tcm, re-run T-065's M/AL/AS/N/L rows with the
// READONLY key only, plus:
//   RL6.M3  the running container is the new image (RL6_NEW_ID); the replaced image (RL6_OLD_ID) is listed
//   RL6.AL  auth-log's header carries auth-lines=, authz-lines=, enroll-lines= and total-lines=, and the
//           printed [auth]/[authz]/[enroll] lines match its counts (header parsed as key=value, any order)
//   RL3     agents-summary: PASS ownerAssigned rows-without-owner=0, K7=PASS, no FAIL/UNDETERMINED
//   RL6.hash Rivet's sha256sum line (RL6_HASHES, 12 hex each) equals the candidate blobs at RL6_SHA
// The parsers self-test on planted output (new format passes; the v1.11.0 header without enroll-lines
// fails) before any tcm read.
// Usage: node rl6.mjs --selftest | RL6_SHA=<sha> RL6_NEW_ID=<12hex> [RL6_OLD_ID=645a6f248d82] RL6_HASHES="<ro12> <k7 12>" node rl6.mjs <outdir>
import { spawnSync, execFileSync } from "node:child_process";
import { createHash } from "node:crypto";
import { writeFileSync, mkdirSync } from "node:fs";
import { join } from "node:path";

// ---------------------------------------------------------------- RL6's own parsers, and their selftest
const OLD_ID = process.env.RL6_OLD_ID ?? "645a6f248d82"; // v1.11.0, the image Loop 6 replaces
...
function rl2AL(out) {
  const hl = out.split("\n").find((l) => /^auth-lines=\d+ /.test(l)) ?? null;
  const kv = Object.fromEntries((hl ?? "").split(/\s+/).map((t) => t.split("=")).filter((x) => x.length === 2));
  const lines = out.split("\n");
  const pa = lines.filter((l) => /\[auth\]/.test(l)).length, pz = lines.filter((l) => /\[authz\]/.test(l)).length, pe = lines.filter((l) => /\[enroll\]/.test(l)).length;
  const nums = ["auth-lines", "authz-lines", "enroll-lines", "total-lines"].every((k) => /^\d+$/.test(kv[k] ?? ""));
  return { ok: nums && +kv["auth-lines"] === pa && +kv["authz-lines"] === pz && +kv["enroll-lines"] === pe, obs: { header: hl, printedAuth: pa, printedAuthz: pz, printedEnroll: pe } };
}
function rl3(out) {
...
if (process.argv[2] === "--selftest") {
  ...
  const neg = { // v1.10.0 shapes, and the failure lines the new script can print
    ...
    al: `auth-lines=1 authz-lines=0 first=${ts} last=${ts} total-lines=9\n${ts} [auth] WOULD REJECT x`,
    alMiscount: `auth-lines=1 authz-lines=1 enroll-lines=2 first=${ts} last=${ts} total-lines=9\n${ts} [auth] x\n${ts} [authz] a\n${ts} [enroll] ISSUE issuer=aaron`,
    as: `aaron\tkind=human\nPASS names-unique\nrows=8 K7=PASS`,
  ...
  const r = [["M3 pos", rl2M3(pos.img, NEW).ok, true], ["M3 neg", rl2M3(neg.img, NEW).ok, false],
    ["AL pos", rl2AL(pos.al).ok, true], ["AL neg (v1.11.0 header: no enroll-lines)", rl2AL(neg.al).ok, false], ["AL neg (miscount)", rl2AL(neg.alMiscount).ok, false],
...
if (!/^[0-9a-f]{7,40}$/.test(process.env.RL6_SHA ?? "")) throw new Error("RL6_SHA not set");
const NEW_ID = process.env.RL6_NEW_ID; if (!/^[0-9a-f]{12}$/.test(NEW_ID ?? "")) throw new Error("RL6_NEW_ID (12 hex) not set");
{ const st = spawnSync(process.execPath, [process.argv[1], "--selftest"], { encoding: "utf8" });
  if (st.status !== 0) { console.log(st.stdout + st.stderr); throw new Error("RL6 selftest failed; no tcm read"); } }
...
{ const r = menuOut.image; check("M3 image", r.rc === 0 && r.out.includes(OLD_ID) && !/\b[0-9a-f]{64}\b/.test(r.out), { rc: r.rc, hasOld: r.out.includes(OLD_ID), lines: r.out.trim().split("\n") }); }
...
{ const r = menuOut.image, x = rl2M3(r.out, NEW_ID); check("RL6.M3 running container is the new image", r.rc === 0 && x.ok, { rc: r.rc, ...x.obs }); }
{ const r = menuOut["auth-log"], x = rl2AL(r.out); check("RL6.AL auth/authz/enroll-lines header, counts match printed lines", r.rc === 0 && x.ok, { rc: r.rc, ...x.obs }); }
{ const r = menuOut["agents-summary"], x = rl3(r.out); check("RL3 ownerAssigned rows-without-owner=0", r.rc === 0 && x.ok, { rc: r.rc, ...x.obs }); }
{ const h = (process.env.RL6_HASHES ?? "").trim().split(/\s+/);
  const blob = (f) => createHash("sha256").update(execFileSync("git", ["show", `${process.env.RL6_SHA}:scripts/tcm/${f}`])).digest("hex").slice(0, 12);
  const exp = [blob("a2a-readonly"), blob("a2a-k7.mjs")];
  check("RL6.hash (Rivet's sha256sum vs candidate blobs)", h[0] === exp[0] && h[1] === exp[1], { reported: h, expected: exp, sha: process.env.RL6_SHA }); }
