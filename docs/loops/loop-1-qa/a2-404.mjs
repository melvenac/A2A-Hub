...
import { hub, key, check, results } from "./harness.mjs";
...
const ctlReads = await hub("GET", `/a2a/session/${live}/reads`, undefined, { headers: { "X-Agent-Key": key(a) } });
...
const r = await hub("GET", `/a2a/session/${GHOST}/reads`, undefined, { headers: { "X-Agent-Key": key(b) } });
