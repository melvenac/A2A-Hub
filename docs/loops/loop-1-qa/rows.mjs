...
import { talk, hub, key, asSeat, reads, messages, cq, cursorFile, sha256OrAbsent, check, results, HUB, DIRECT } from "./harness.mjs";
...
if (want.has("A1")) {
  ...
  const m1 = (await messages(sid, a)).find((m) => m.turn === 1);
  let r = await reads(sid, a);
...
if (want.has("A2")) {
  ...
  for (const mode of ["inbox", "wait"]) {
    ...
    await proxyReset();
    const r3 = await reads(sid3, e);
    const msgs3 = await messages(sid3, e);
  ...
  const markOf = async () => P(await reads(sid4, g), h)?.lastRead ?? null;
  ...
  for (const [name, sidX] of [["unknown-session-other-table-id", otherTableId], ["unknown-session-garbage", "not-a-session-id"]]) {
    ...
    const rr = await hub("GET", `/a2a/session/${sidX}/reads`, undefined, { headers: asSeat(h) });
...
if (want.has("A3")) {
  ...
  const snapRaw = (await reads(sid, a)).text;
  const snapMarks = marks(await reads(sid, a));
  ...
  const seen = [];
  // Logged by label, never by value: a seat's key must not reach a run log (K6).
  for (const [label, headers] of [["b-key", asSeat(b)], ["dev-key", { "X-Agent-Key": "dev-key" }], ["no-key", {}]]) {
    for (const g of gets) seen.push(`${(await hub("GET", g, undefined, { headers })).status} ${label} ${g.replace(sid, ":sid")}`);
  }
  ...
  r = await reads(sid, a);
  const msgsAfter = await messages(sid, a);
