...
// The probe's throwaway key (Loop 3 design §8): fresh per run, over the 32-character floor.
const KEY = `qa-probe-${randomBytes(16).toString("hex")}`;
...
const reg = await fetch(`${HUB}/a2a/register`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: "qa-probe", apiKey: KEY, agentCard: { name: "qa-probe", description: "QA isolation probe", kind: "qa-probe" } }) });
const after = { agents: (await q("agents:listOnline")).map(a => a.name), probe: (await q("agents:getByName", { name: "qa-probe" }))?.name ?? null };
const hubSessions = await fetch(`${HUB}/a2a/sessions`, { headers: { "X-Agent-Key": KEY } }).then(r => r.json());
