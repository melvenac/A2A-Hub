...
import { talk, hub, messages, register, room, cursorFile, check, results, DIRECT, HUB } from "./harness.mjs";
...
const say1 = await talk(HT, ["--as", A, "--session", sid, "--say", "t1"], { hub: DIRECT });
const wait1 = await talk(HT, ["--as", B, "--session", sid, "--wait", "--wait-timeout", "10"], { hub: DIRECT });
...
const to = await talk(HT, ["--as", B, "--session", sid, "--wait", "--wait-timeout", "3"], { hub: DIRECT });
check("H.exit2", "--wait with no new turn exits 2", to.code, to.code === 2);

// X pass-through: identical to direct.
await talk(HT, ["--as", A, "--session", sid, "--say", "t2"], { hub: HUB });
const viaDirect = await talk(HT, ["--as", B, "--session", sid, "--inbox"], { hub: DIRECT });
const viaProxy = await talk(HT, ["--as", B, "--session", sid, "--inbox"], { hub: HUB });
...
const directReads = await hub("GET", `/a2a/session/${sid}/reads`);
await mode({ reads: "500" });
const proxReads = await hub("GET", `/a2a/session/${sid}/reads`, undefined, { base: HUB });
...
let dropped;
try { await hub("POST", `/a2a/session/${sid}/read`, { reader: B, throughTurn: 1, via: "wait" }, { base: HUB }); dropped = "answered"; } catch (e) { dropped = `socket error (${e.cause?.code ?? e.message})`; }
check("X.drop", "read=drop destroys the socket", dropped, dropped.startsWith("socket error"));
await mode({ read: "delay:2000" });
const t0 = Date.now(); await hub("POST", `/a2a/session/${sid}/read`, { reader: B, throughTurn: 1, via: "wait" }, { base: HUB });
...
await hub("POST", `/a2a/session/${sid}/read`, { reader: B, throughTurn: 2, via: "inbox" }, { base: HUB });
